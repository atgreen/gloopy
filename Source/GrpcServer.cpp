...
namespace
{
    ...
    public:
        ...
        { const bool ok = main.apiSetSamplerControls (q->track_id(), q->start(), q->end(), q->reverse(), q->root_note(), q->fade_in(), q->fade_out(), q->loop());
        ...
        { const auto s = main.apiGetSamplerControls (q->id());
          ...
          r->set_fade_in (s.fadeIn); r->set_fade_out (s.fadeOut); r->set_loop (s.loop); return Status::OK; }
        ...
        { bool en=false, hold=false; double rate=0.25; int oct=1, mode=0; float gate=0.5f, swing=0.0f;
          if (main.apiGetTrackArp (q->track_id(), en, rate, oct, gate, mode, swing, hold))
