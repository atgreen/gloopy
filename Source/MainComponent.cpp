    ...
    : headlessCli (headless)
{
    ...
    cats.push_back ({ "Favs",
        [this]
        {
            browserFavorites = apiListFavorites();
            std::vector<juce::String> names;
            for (auto& f : browserFavorites) names.push_back (f.label);
            return names;
        },
        [this, dispatchFavorite] (const juce::String& label)
        {
            for (auto& f : browserFavorites) if (f.label == label) { dispatchFavorite (f); return; }
        },
        [this] (const juce::String& label)
        {
...
public:
          ...
          okCb (std::move (onOk)), cancelCb (std::move (onCancel))
    {
        ...
        setWantsKeyboardFocus (true);
        juce::Component::SafePointer<FormDialog> sp { this };
        juce::MessageManager::callAsync ([sp] { if (sp != nullptr) sp->focusFirst(); });
    }
...
void MainComponent::showRemoteMenu()
{
    ...
    m.showMenuAsync (juce::PopupMenu::Options().withTargetComponent (fileButton),
        [this, dir, remote, branch, doNet] (int r)
        {
            if (r == 1)
            {
                auto dismiss = [this] { juce::Component::SafePointer<MainComponent> sp { this }; juce::MessageManager::callAsync ([sp] { if (sp != nullptr) sp->formDialog.reset(); }); };
...
void MainComponent::showCommitDialog()
{
    ...
    auto dismiss = [this] { juce::Component::SafePointer<MainComponent> sp { this }; juce::MessageManager::callAsync ([sp] { if (sp != nullptr) sp->commitDialog.reset(); }); };
...
juce::ValueTree MainComponent::toValueTree()
{
    ...
    if (arrangeView) { root.setProperty ("viewPxPerBeat", arrangeView->getPxPerBeat(), nullptr);       // timeline zoom / scroll (0 = fit)
                       ...
                       root.setProperty ("viewTrackScale", arrangeView->getTrackHeightScale(), nullptr);   // vertical zoom
                       root.setProperty ("viewWaveAmp",    arrangeView->getWaveAmpScale(), nullptr); }     // waveform amplitude
...
void MainComponent::loadFromTree (const juce::ValueTree& root)
{
    ...
    if (arrangeView) arrangeView->setViewState ((double) root.getProperty ("viewPxPerBeat", 0.0),
                                                ...
                                                (double) root.getProperty ("viewTrackScale", 1.0),
                                                (double) root.getProperty ("viewWaveAmp", 1.0));
